#Allow R to read from librarys stored in Downloads folder
...
library(shiny)
library(rpart)
install.packages("C:\\Users\\blodge9\\Downloads\\RGtk2_2.20.31.zip", repos= NULL, type="source")
library(RGtk2)
...
#Example Google Stock Price (end of day) and Volume traded (end of day)
GOOGL.df <- as.data.frame(bdh("GOOGL US Equity", c("PX_LAST", "Volume", "PX_Open", "PX_High", 
                                                   "PX_Low", "PX_DIV_ADJ_CLOSE_1D"), start.date=Sys.Date()-31))
#import export 
View(GOOGL.df)

write.csv(GOOGL.df, file = "googlestock.csv")
...
#plot the timeseries of google stock price
plot(GOOGL.df$date, GOOGL.df$PX_LAST, type="o")

#Distribution of price
boxplot(GOOGL.df$PX_LAST)

#Distirbution of volume see outliers
boxplot(GOOGL.df$Volume)

#ggplot
ggplot(GOOGL.df, aes(date, PX_LAST)) + geom_line()  
...
print(system.file("examples", package="shiny"))
...
fit <- rpart(x ~ Pclass + Sex + Age + SibSp + Parch + Fare + Embarked,
             data=train,
             method="class")
