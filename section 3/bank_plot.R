...
qplot(bank$loan, bank$age, data = bank, geom = c("point","smooth"), color = factor(y))

bank_full$month <- as.factor(bank_full$month)
prop.table(table(bank_full$month, bank_full$y))

ggplot(data=bank_full) +
  geom_histogram(aes(x=month, fill=y),position = "dodge", binwidth = 5)
ggtitle("Distribution of age by outcome of marketing call")

names(bank_full)
sm.density.compare(bank_full$age,bank$y)
colfill<-c(2:(2+length(levels(bank_full$y))))
legend("topright", levels(bank_full$y), fill=colfill)
